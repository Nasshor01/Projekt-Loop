...
func _draw():
	...
	if _is_valid_grid_position(_mouse_over_cell):
...
func place_object_on_cell(object_node: Node2D, grid_position: Vector2i, is_moving: bool = false) -> bool:
	if not _is_valid_grid_position(grid_position) or _grid_objects.has(grid_position): return false
...
func show_movable_range(from_cell: Vector2i, move_range: int):
	_movable_cells.clear(); for x in range(-move_range, move_range + 1):
		for y in range(-move_range, move_range + 1):
			...
			if _is_valid_grid_position(target_cell) and get_object_on_cell(target_cell) == null: _movable_cells.append(target_cell)
	queue_redraw()
